...
import { getCountUser, verify } from "../../store/Auth/AuthSlice";
import {
  getOrders,
  getSumMoney,
  getSumMoneyToday,
...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    dispatch(getSumMoney());
    dispatch(getSumMoneyToday());
    dispatch(getCountUser());
  }, [dispatch]);

  const { orders, sumMoney, sumMoneyToday } = useSelector(
    (state) => state.orders
  );
  const { productSold } = useSelector((state) => state.products);
  const { countUser } = useSelector((state) => state.auth);
  const dataSource =
    orders?.length > 0
      ? orders.map((order) => {
          return {
            ...
            "Thành tiền": converseMoney(order?.sumPayment),
  ...
  const dataSourceProducts =
    productSold?.length > 0
      ? productSold?.map((product) => {
          return {
            nameProduct: product.nameProduct,
            price: converseMoney(product.price),
            image: product.image,
            sold: product.sold,
            key: product.id,
          };
  ...
  const count = [
    {
      today: "Doanh thu",
      title: converseMoney(sumMoney) || 0,
      icon: dollor,
      bnb: "bnb2",
    },
    {
      today: "Doanh thu trong ngày",
      title: converseMoney(sumMoneyToday) || 0,
      icon: profile,
      bnb: "bnb2",
    },
    {
        ...
        <Row gutter={[24, 0]}>
          <Col xs={24} sm={24} md={12} lg={12} xl={24} className="mb-24">
            <Card bordered={false} className="criclebox h-full">
              <div style={{ fontSize: 20 }}>Đơn đặt hàng mới nhất</div>
            ...
            >
              <div style={{ fontSize: 20 }}>
                ...
                <Button style={{ marginLeft: 15 }}>
                  <FundOutlined style={{ marginTop: 6 }} />
                </Button>
              </div>
              <div>
                <Table
